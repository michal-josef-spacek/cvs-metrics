...
if ($parser) {
	...
	my $top = new Top($cvs_log, \@tags, $title, $flg_dead, $flg_css, $start_date);
	MainLoop();
}
...
sub new {
	...
	my $self = {};
	bless($self, $class);
	my ($cvs_log, $tags, $title, $flg_dead, $flg_css, $start_date) = @_;
...
sub _audit {
	...
	if (        $self->{tag_to} ne 'HEAD'
			and $self->{tag_from} ge $self->{tag_to} ) {
		$self->{mw}->messageBox(
	...
	my $html = main::GenerateHTML($self->{cvs_log}, $self->{tags}, $self->{title}, $path, $self->{tag_from}, $self->{tag_to}, $self->{css}, $self->{start_date});
	print "Starting browser...\n";
	system "start $html";
#	use POSIX qw(getcwd);
#	my $uri = "file://" . POSIX::getcwd() . "/" . $html;
#	system "start $uri";
#	system "cmd /s \"C:/Program Files/Internet Explorer/iexplore.exe\" $uri";
#	system "\"C:/Program Files/Netscape/Communicator/Program/netscape.exe\" file://$html";

#	use WIN32::OLE;
#	my $ie = Win32::OLE->new('IExplore.Application') or warn "oops\n";
}
